...
class JSONDiffApp {
    ...
    renderLine(lineNumber, content, node, status, isStructural) {
        const lineContainer = document.createElement('div');
        lineContainer.className = `multi-file-line ${status}`;
        lineContainer.style.display = 'grid';
        lineContainer.style.gridTemplateColumns = `60px 200px repeat(${this.files.length}, 250px)`;
    ...
    nextPage() {
        const totalPages = Math.ceil(this.allLines.length / this.itemsPerPage);
    ...
    renderCurrentPage() {
        ...
        const pageLines = this.allLines.slice(start, end);
        
        this.elements.diffViewer.innerHTML = '';
        this.renderInBatches(pageLines, 50);
